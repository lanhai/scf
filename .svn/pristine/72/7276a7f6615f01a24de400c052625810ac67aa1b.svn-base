    ...
    <el-table :data="dataList" stripe style="width: 100%" row-key="id">
      ...
      <el-table-column label="操作">
        <template
          ...
          <el-button type="text" class="c-red" @click="(scope.$index)" icon="el-icon-delete">删除</el-button>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    submit() {
      this.$refs['form'].validate((valid) => {
        this.isLoading = true;
        if (valid) {
          this.$api.render.post('{saveUrl}', this.$copy(this.input)).then(result => {
            if (result.errCode) {
    ...
    remove(index) {
      ...
      this.$message.confirm({message: '确定要删除吗?'}, () => {
        ...
        this.$api.render.post('{removeUrl}', {id: item.id}).then(result => {
          if (result.errCode) {
            this.$message.alert(result.message)
          } else {
    ...
    getList() {
      ...
      this.$api.render.get('{listUrl}', searchParams).then(result => {
        if (result.errCode) {
