...
class Struct {
     ...
     * @return bool
     */
    public function create(array $data, bool $validate = true): bool {
        $fields = $this->_getFields();
        $_data = [];
     ...
     * 分析验证规则
     */
    protected function _parseValidateRules() {
        $fields = $this->_getFields();
        foreach ($fields as $f) {
            $name = $f->getName(); // 字段名称
                ...
                for ($i = 0; $i < count($matches[0]); $i++) {
                    ...
                    switch ($rn) {
                        ...
                        case 'rule':
                            ...
                            $rc[0] = trim($rc[0]);
                            $rule = [];
                            Filter::registerFilter('chs_string', '\Scf\Database\Filters\ChineseString');
