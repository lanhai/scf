...
class CpCreater extends Base {
    ...
    protected function createController(): mixed {
        ...
        //模板文件选择
        if ($tplType == 1) {
            $controllerFileContent = file_get_contents(SCF_ROOT . '/lib/scf/Command/Template/CpViewTpl.vue');
            ...
            if ($arFile) {
                ...
                try {
                    ...
                    $columns = $db->connection()->exec($sql)->get();
...
//                        ]
            ...
            $this->setConfig('auth_action_path', StringHelper::lower2camel($apiModuleName) . "/" . StringHelper::camel2lower($apiControllerName) );
            $this->setConfig('api_controller_name', StringHelper::camel2lower($apiControllerName));
            $apiControllerFileContent = file_get_contents(SCF_ROOT . '/lib/scf/Command/Template/cpController.tpl');
            ...
            $controllerFileContent = file_get_contents(SCF_ROOT . '/lib/scf/Command/Template/CpDataListTpl.vue');
        }
    ...
    protected function createAuthNode(): mixed {
        $result = \App\Admin\Model\AuthNodeModel::instance()->all(['parent' => 0]);
        if (!$result) {
        ...
        $node = \App\Admin\Model\AuthNodeAR::factory();
        $node->parent = $parent;
