...
class ArrayHelper {
     ...
     * @return array|mixed
     */
    public static function integer($array): mixed {
        if (!is_array($array) || !array_is_list($array)) {
            return $array;
        }
        $numbers = [];
        array_map(
            function ($val) use (&$numbers) {
                $numbers[] = is_numeric($val) ? (int)$val : $val;
            },
            $array
        );
        return $numbers;
     ...
     * @return string
     */
    public static function toXml(array $arr): string {
        $xml = "<xml>";
     ...
     * @return string
     */
    public static function toQueryMap(array $paraMap, bool $urlencode = true): string {
        $buff = "";
        ksort($paraMap);
     ...
     * @return int
     */
    public static function level($arr): int {
        $al = array(0);
     ...
     * @return mixed|null
     */
    public static function findColumn($array, $key, $value): mixed {
        if (!is_array($array)) {
            return null;
        }
     ...
     * @return array|object the array representation of the object
     */
    public static function toArray(object|array|string $object, array $properties = [], bool $recursive = true): array|object {
        if (is_array($object)) {
            if ($recursive) {
     ...
     */
    public static function index(array $array, \Closure|string|null $key, array|string $groups = []): array {
        $result = [];
        $groups = (array)$groups;
     ...
     */
    public static function getColumn(array $array, \Closure|string $name, bool $keepKeys = true): array {
        $result = [];
        if ($keepKeys) {
     ...
     * @return array
     */
    public static function map(array $array, \Closure|string $from, \Closure|string $to, \Closure|string $group = null): array {
        $result = [];
        foreach ($array as $element) {
