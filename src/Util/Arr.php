...
class Arr {
     ...
     * @param string $type 排序方式 asc:反序,desc:正序
     * @param bool $newkey 是否覆盖key值
     * @return array
     */
    public static function sort(array $arr, string $keys, string $type = 'asc', bool $newkey = false): array {
     ...
     * @param mixed|null $default default value if the path is not set
     * @param string|null $delimiter key path delimiter
     * @return  mixed
     */
    public static function path(array $array, mixed $path, mixed $default = null, string $delimiter = null): mixed {
            ...
            if (isset($array[$key])) {
                if ($keys) {
                    if (Arr::isArray($array[$key])) {
                        return Arr::path($array[$key], $keys, $default, $delimiter);
                    } else {
                        return $array[$key];
     ...
     * @see Arr::path()
     */
    public static function setPath(array &$array, mixed $path, mixed $value, string $delimiter = null) {
        if (!$delimiter) {
            // Use the default delimiter
            $delimiter = Arr::$delimiter;
        }
     ...
     * @return  array
     */
    public static function extract(array $array, array $paths, mixed $default = null): array {
        $found = array();
     ...
     * @return  array
     */
    public static function unshift(array &$array, string $key, mixed $val): array {
     ...
     * @return  array
     */
    public static function map(mixed $callbacks, array $array, array $keys = null): array {
        foreach ($array as $key => $val) {
            if (is_array($val)) {
                $array[$key] = Arr::map($callbacks, $val);
     ...
     * 遍历树形结构的数组, 模拟递归算法
     * @param $array
     * @param \Closure $callback function($row, $index, $level, $list, $array)
     * @param \Closure|null $cbkIn 进入下级回调函数 function($row)
     * @param \Closure|null $cbkOut 退出下级回调函数
     * @param string $childrenKey
     */
    public static function mapTree($array, \Closure $callback, \Closure $cbkIn = null, \Closure $cbkOut = null, string $childrenKey = 'children') {
        $stack = []; // [index, list]
        $stack[] = [0, $array];
        ...
        while (true) {
            ...
            // 执行回调
            $callback($row, $index, $level, $list, $array);
            ...
            if ($index < count($list)) {
                ...
                $cbkOut and $stack and $cbkOut(); // 退出下级
            }

            if (isset($row[$childrenKey])) {
                // 添加新环境
                $cbkIn and $cbkIn(); // 进入下级
                $stack[] = [0, $row[$childrenKey]];
            }
        }
    }
