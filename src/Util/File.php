...
class File {
     ...
     */
    public static function readJson($file, int $len = -1): bool|string|array {
        $content = self::read($file, $len);
        if ($content && JsonHelper::is($content)) {
            $content = JsonHelper::recover($content);
        }
        return $content;
    }
     ...
     * @return false|string
     */
    public static function read($file, int $len = -1): bool|string {
        if (!file_exists($file)) {
            return false;
        }
        $fp = fopen($file, "r");
     ...
     * @return bool
     */
    public static function write($file, $content, bool $append = false): bool {
        if ($append) {
            $fp = fopen($file, "a+");
            $content .= "\n";
        } else {
     ...
     * @throws Exception
     */
    public static function copyDirectory(string $src, string $dst, array $options = array()) {
        $fileTypes = array();
        $exclude = array();
        $level = -1;
        extract($options);
        if (!is_dir($dst))
            self::createDirectory($dst, $options['newDirMode'] ?? null, true);
     ...
     * @since 1.1.14
     */
    public static function removeDirectory(string $directory, array $options = array()) {
     ...
     * @throws Exception
     */
    public static function findFiles(string $dir, array $options = array()): array {
        $fileTypes = array();
        $exclude = array();
        $level = -1;
        $absolutePaths = true;
        extract($options);
        $list = self::findFilesRecursive($dir, '', $fileTypes, $exclude, $level, true);
        sort($list);
        return $list;
    }
     ...
     * @throws Exception
     */
    protected static function copyDirectoryRecursive(string $src, string $dst, string $base, array $fileTypes, array $exclude, int $level, array $options) {
        if (!is_dir($dst))
            self::createDirectory($dst, $options['newDirMode'] ?? null, false);
     ...
     * @throws Exception
     */
    protected static function findFilesRecursive(string $dir, string $base, array $fileTypes, array $exclude, int $level, bool $absolutePaths): array {
        $list = array();
     ...
     */
    protected static function validatePath(string $base, string $file, bool $isFile, array $fileTypes, array $exclude): bool {
     ...
     * @param string|null $magicFile name of a magic database file, usually something like /path/to/magic.mime.
     ...
     * @param string $file the file name.
     * @param string|null $magicFile the path of the file that contains all available MIME type information.
     ...
     * @param integer|null $mode the permission to be set for newly created directories, if not set - 0777 will be used
