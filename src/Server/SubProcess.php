...
class SubProcess {
     ...
     * @return Process
     */
    public static function heartbeat($server, Node $node): Process {
        return new Process(function ($process) use ($server, $node) {
            ...
            function report($server, $node): void {
                $manager = ServerManager::instance();
                try {
                    if ($manager->report($node) === false) {
                        Console::log('【Server】节点报道失败:' . Color::red("MasterDB不可用"));
                        Timer::after(5000, function () use ($server, $node) {
                            report($server, $node);
                        });
                    }
                    Timer::tick(1000, function () use ($manager, $node, $server) {
     ...
     * @return Process
     */
    public static function createLogBackupProcess($server): Process {
     ...
     * @return Process
     */
    public static function createFileWatchProcess($server, int $port = 0): Process {
