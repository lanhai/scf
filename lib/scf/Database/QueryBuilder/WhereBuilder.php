...
namespace Scf\Database\QueryBuilder;
...
class WhereBuilder {
    ...
    public function getWhereSql(): ?string {
        ...
        for ($i = 0; $i < count($where['match']); $i++) {
            $in = $where['match'][$i];
            if (!is_array($in)) {
                $in = is_numeric($in) || str_contains($in, "AND") ? $in : "'{$in}'";
            } else {
                $in = implode(',', $in);
            }
